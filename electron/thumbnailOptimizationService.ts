...
import { announceCachedThumbnail, ensureThumbnailPath, getThumbnailCacheFileInventory } from "./thumbnailService";
...
const announcedCacheKeys = new Set<string>();
...
const cachedAnnouncementBatchSize = 128;
  ...
  try {
    while (enabled && pauseReasons.size === 0) {
      ...
      try {
        ...
        announcedCacheKeys.add(candidate.cacheKey);
...
export const enqueueThumbnailOptimizationCandidates = async (candidates: ThumbnailOptimizationCandidate[]) => {
  ...
  const enqueueRevision = enabledRevision;

  const cacheFileInventory = await getThumbnailCacheFileInventory();
  if (!enabled || enqueueRevision !== enabledRevision) {
    return;
  }

  let addedCandidate = false;
  let announcedCachedCount = 0;
  for (const candidate of candidates) {
    if (!enabled || enqueueRevision !== enabledRevision) return;
    const candidateKey = normalizePathKey(candidate.filePath);
    ...
    if (
      ...
      if (announcedCacheKeys.has(cacheEntry.key)) continue;
      announcedCacheKeys.add(cacheEntry.key);
      announceCachedThumbnail(
        candidate.filePath,
        cacheEntry.imagePath,
        candidate.fileSize,
        candidate.modifiedMs
      );
      announcedCachedCount += 1;
      if (announcedCachedCount % cachedAnnouncementBatchSize === 0) {
        await yieldToForegroundWork();
      }
      continue;
    }
