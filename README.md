...
> **Cap7CE 1.0.3.** Stable Windows x64 release. Back up important files before using file-management features.
...
请从 [GitHub Releases](https://github.com/7C93F3-L/Cap7CE/releases/latest) 下载 `Cap7CE-Setup-1.0.3-x64.exe`。安装器支持选择最终安装目录、创建开始菜单和桌面快捷方式，以及覆盖安装；不会在用户选择的路径下再自动添加 `Cap7CE` 子目录。软件内更新只在用户主动检查和确认后下载、校验并启动正式安装器。
