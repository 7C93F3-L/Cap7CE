...
const searchResultsStyles = read("src/renderer/stable-ui/StableSearchResults.css");
...
assert.match(sidebarStyles, /\.cap-stable-directory-tooltip \{[^}]*animation: cap-stable-directory-tooltip-enter 120ms cubic-bezier\(0\.22, 0\.85, 0\.18, 1\) both;/u);
assert.match(sidebarStyles, /@keyframes cap-stable-directory-tooltip-enter \{[\s\S]*?opacity: 0; transform: translate3d\(-5px, 0, 0\);[\s\S]*?opacity: 1; transform: translate3d\(0, 0, 0\);/u);
...
assert.match(sidebarStyles, /\.cap-stable-directory-row:hover \{ background: var\(--cap-stable-control-hover\); \}[\s\S]*?\.cap-stable-directory-row:active \{ background: var\(--cap-stable-control-pressed\); \}[\s\S]*?\.cap-stable-directory-row\.is-selected \{[^}]*background: var\(--cap-accent-gradient\); \}[\s\S]*?\.cap-stable-directory-row > \.cap-stable-directory-item\.is-selected \{ background: transparent; \}/u);
...
assert.match(materialContrastStyles, /\.cap-stable-ui:not\(\.theme-dark\)\[data-window-material="acrylic"\],[\s\S]*?--cap-stable-hover: rgb\(31 31 31 \/ 10%\);[\s\S]*?--cap-stable-control-hover: rgb\(31 31 31 \/ 15%\);[\s\S]*?--cap-stable-control-pressed: rgb\(31 31 31 \/ 20%\);/u);
assert.match(sidebarStyles, /\.cap-stable-sidebar-control:where\(:hover, :focus-visible\) \{ background: var\(--cap-stable-control-hover\); \}[\s\S]*?\.cap-stable-sidebar-control:active \{ background: var\(--cap-stable-control-pressed\); \}[\s\S]*?aria-pressed="true"[\s\S]*?aria-expanded="true"[^}]*background: var\(--cap-accent-gradient\);/u);
assert.match(sidebarStyles, /\.cap-stable-sidebar-footer button:where\(:hover, :focus-visible\) \{ background: var\(--cap-stable-control-hover\); \}[\s\S]*?button:active \{ background: var\(--cap-stable-control-pressed\); \}[\s\S]*?\.cap-stable-sidebar-footer button\.is-active \{ color: var\(--dialog-action-hover-text, var\(--theme-on-color\)\); background: var\(--cap-accent-gradient\); \}/u);
assert.match(sidebarStyles, /\.cap-stable-sidebar-control,[\s\S]*?\.cap-stable-directory-item \{[^}]*transition: var\(--cap-stable-control-transition\);/u);
assert.match(sidebarStyles, /\.cap-stable-directory-heading button \{[^}]*transition: var\(--cap-stable-control-transition\);[^}]*\}[\s\S]*?button:where\(:hover, :focus-visible\) \{ background: var\(--cap-stable-control-hover\); \}[\s\S]*?button:active \{ background: var\(--cap-stable-control-pressed\); \}/u);
assert.match(sidebarStyles, /\.cap-stable-directory-row \{[^}]*transition: var\(--cap-stable-control-transition\);[^}]*\}[\s\S]*?\.cap-stable-directory-row:hover \{ background: var\(--cap-stable-control-hover\); \}[\s\S]*?\.cap-stable-directory-row:active \{ background: var\(--cap-stable-control-pressed\); \}/u);
assert.match(sidebarStyles, /\.cap-stable-sidebar-footer button \{[^}]*transition: var\(--cap-stable-control-transition\);/u);
assert.match(searchResultsStyles, /\.cap-stable-search-clear \{[^}]*background: var\(--cap-stable-hover\);[^}]*transition: var\(--cap-stable-control-transition\);[^}]*\}[\s\S]*?\.cap-stable-search-clear:hover \{ background: var\(--cap-stable-control-hover\); \}[\s\S]*?\.cap-stable-search-clear:active \{ background: var\(--cap-stable-control-pressed\); \}/u);
...
assert.match(navigationStateStyles, /--cap-stable-control-transition: background 180ms ease, color 180ms ease;/u);
assert.match(navigationStateStyles, /--cap-stable-flyout-enter: cap7ce-context-menu-enter 180ms cubic-bezier\(0\.22, 0\.85, 0\.18, 1\) both;/u);
...
assert.match(sidebarStyles, /\.cap-stable-sidebar-flyout \{[^}]*animation: var\(--cap-stable-flyout-enter\);/u);
assert.match(sidebarStyles, /\.cap-stable-sidebar-flyout button \{[^}]*border-radius: 999px;/u);
assert.match(sidebarStyles, /\.cap-stable-sidebar-flyout button \{[^}]*transition: var\(--cap-stable-control-transition\);/u);
assert.match(sidebarStyles, /\.cap-stable-sidebar-flyout button:where\(:hover, :focus-visible\):not\(:disabled\) \{ background: var\(--cap-stable-control-hover\); \}[\s\S]*?button:active:not\(:disabled\) \{ background: var\(--cap-stable-control-pressed\); \}[\s\S]*?button\.is-selected \{[^}]*background: var\(--cap-accent-gradient\); \}/u);
...
assert.match(sidebarStyles, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.cap-stable-sidebar-flyout \{ animation: none; \}[\s\S]*?\.cap-stable-directory-tooltip \{ animation: none; \}[\s\S]*?cap-stable-sidebar-flyout button \{ transition: none; \}/u);
