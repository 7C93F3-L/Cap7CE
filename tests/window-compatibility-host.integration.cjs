...
assert.match(mainSource, /windowPresentationRuntime\.configure\(await windowPresentationSwitchRuntime\.resolveStartupMode\(normalizeWindowPresentationMode\(requestedWindowPresentationMode\)\), preferences\.themePreference\);[\s\S]*?windowLayoutManager = new WindowLayoutManager\(new WindowLayoutStore\(path\.join\(app\.getPath\("userData"\), "config", windowPresentationRuntime\.layoutFileName\)\)\);[\s\S]*?await windowLayoutManager\.load\(\);/u);
assert.match(mainSource, /new BrowserWindow\(\{[\s\S]*?\.\.\.getMainWindowPresentationOptions\(\)[\s\S]*?paintWhenInitiallyHidden: true/u);
