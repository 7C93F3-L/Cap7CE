...
assert.match(mainSource, /defaultBounds: \(candidate\) => resolveStableUiDefaultWindowBounds\(candidate\.workArea\)/u);
assert.match(mainSource, /minimumSize: STABLE_UI_MINIMUM_OUTER_SIZE/u);
...
assert.match(mainSource, /const shortcutModes = \[[\s\S]*?activateStandby[\s\S]*?activateSkim[\s\S]*?openSettings[\s\S]*?activateNormal/u);
assert.match(mainSource, /getActiveShortcutActions[\s\S]*?preferences\.stableShortcutActions[\s\S]*?preferences\.shortcutActions/u);
assert.match(mainSource, /activateMainSearchShortcut[\s\S]*?dockedShellController\?\.restore\(false\)[\s\S]*?showAndFocusMainWindow/u);
assert.match(mainSource, /const activateMainSearchShortcut[\s\S]*?sendFocusMainSearchToRenderer/u);
...
assert.doesNotMatch(quickActionsSource, /activateMicro|activateMini/u);
...
assert.doesNotMatch(hintSource, /activateMicro|activateMini/u);
...
assert.match(previewTitlebarStyles, /preview-window-stable-ui\s*\{[\s\S]*?--preview-titlebar-height: 40px;[\s\S]*?border: 0;[\s\S]*?border-radius: 0;[\s\S]*?background: transparent;/u);
