...
const previewAppearanceSource = read("src/renderer/preview/usePreviewAppearance.ts");
...
assert.match(previewSource, /usePreviewAppearance\(previewData\)/u);
assert.match(previewAppearanceSource, /useSystemThemeMode\(\)[\s\S]*?effectiveTheme = themePreference === null[\s\S]*?previewData\?\.theme \?\? systemTheme[\s\S]*?themePreference === "system" \? systemTheme : themePreference/u);
assert.match(previewAppearanceSource, /effectiveAppearanceColors = appearanceColors \?\? previewData\?\.appearanceColors[\s\S]*?--preview-action-hover-text": getTextColorForBackground\(effectiveAppearanceColors\.themeColor, effectiveAppearanceColors\.accentColor\)/u);
assert.match(previewAppearanceSource, /useUiFontSize\(uiFontSize\)/u);
assert.match(previewAppearanceSource, /const applyPreferences[\s\S]*?setThemePreference\(preferences\.themePreference\);[\s\S]*?setAppearanceColors\(preferences\.appearanceColors\);[\s\S]*?setUiFontSize\(preferences\.uiFontSize\);[\s\S]*?setWindowMaterial\(preferences\.windowMaterial\);[\s\S]*?preferences\.onChanged\(applyPreferences\)/u);
