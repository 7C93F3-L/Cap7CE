...
assert.match(sharedMenuStyleSource, /windowMaterial: WindowMaterial[\s\S]*?"--context-menu-control-hover":[^\n]*windowMaterial === "acrylic" \? "rgb\(31 31 31 \/ 15%\)"[\s\S]*?"--context-menu-control-pressed":[^\n]*windowMaterial === "acrylic" \? "rgb\(31 31 31 \/ 20%\)"/u);
assert.match(appSource, /getFileContextMenuStyle\(effectiveTheme, appearanceColors, windowMaterial\)/u);
assert.match(responsiveSkimMenuSource, /getFileContextMenuStyle\(theme, appearanceColors, windowMaterial\)/u);
assert.match(responsiveMenuStyles, /\.responsive-file-context-menu \{[^}]*background: var\(--cap-stable-flyout-surface\);/u);
assert.match(responsiveMenuStyles, /\.responsive-file-context-menu-info strong \{[^}]*justify-self: start;[^}]*border-radius: 999px;[^}]*color: var\(--dialog-action-hover-text\);[^}]*background: var\(--cap-accent-gradient\);[^}]*font-weight: 700;/u);
...
assert.match(responsiveMenuStyles, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.responsive-file-context-menu \{ animation: none; \}[\s\S]*?responsive-file-context-menu-actions button \{ transition: none; \}/u);
