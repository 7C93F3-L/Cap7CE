...
const document = { version: 1, lastDockEdge: null, lastDockDisplayId: null, profiles: { normal: profile } };
const writes = [];
...
const resolve = () => manager.resolveBounds({
  state: "normal",
  displays: [display],
...
const run = async () => {
  ...
  assert.deepEqual(resolve(), { x: 320, y: 140, width: 1280, height: 760 });
  ...
  assert.deepEqual(resolve(), { x: 100, y: 120, width: 900, height: 600 });
  ...
  assert.equal(writes.length, 1);
  console.log(JSON.stringify({ completeBoundsRestoreVerified: true, userBoundsCaptureVerified: true, standbyLineTaskbarDirectionVerified: true, dockDirectionIndependentFromLineVerified: true, disabledCaptureSkipped: true }));
};
