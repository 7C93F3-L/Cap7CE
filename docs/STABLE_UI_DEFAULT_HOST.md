...
人工确认至少检查：首次或无有效偏好时进入新版稳定 UI；主窗口可自由缩放与拖动；Settings 和 Preview 可与主窗口并存并正常关闭；同一缩略图可重复按空格预览；置顶、line、搜索、目录、Skim 和文件动作无明显回归；三次受控切换可依次进入 compatibility、cap7ce 并回到 stable。人工确认前不提交本轮。
