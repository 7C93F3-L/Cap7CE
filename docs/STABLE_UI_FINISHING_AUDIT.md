...
- 主窗口、Settings 与 Preview 的正式 stable 宿主统一使用可选 Acrylic / Mica、40 DIP Window Controls Overlay；不支持所选材质时使用当前明暗主题的安全纯色。
...
- 既有 920px、560px 与 360px 高度断点保持不变。Preview 信息边栏不参与窄窗连续压缩，仅保留固定 280px 展开与固定 40px 收起两种状态。

## 3. 无障碍、动效与溢出

主窗口两处分隔线支持指针、双击复位以及方向键 / Home / End 调整，并公开当前值与范围。Preview 信息栏使用固定展开宽度，不再暴露无必要的调宽分隔线；折叠按钮在展开和收起时提供明确名称。Settings 当前分类使用 `aria-current`，确认层使用 `alertdialog` 并把初始焦点放在取消动作。
