...
本轮不切换默认宿主、不删除旧 Cap7CE / compatibility 外壳，也不迁移旧布局记录。当前稳定 UI 仍需要仅开发入口完成真实窗口验收；在没有正式入口替代时删除该开关会使候选界面不可进入，因此开关移除必须与“切换默认宿主”在验收后的独立轮次同时决策。
...
本轮不制作安装包。测试包、默认宿主切换、开发入口移除和旧 Renderer / CSS 删除都需要人工验收后再次明确授权。
