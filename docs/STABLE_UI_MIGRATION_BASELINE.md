...
| Settings | U0 仍由 `App.tsx` 的 `openSettings` 与同窗 `settings` shell state 打开，设置读写继续走现有偏好和领域 IPC | `SettingsView` 及其分区组件、`preferenceIpc.ts` | U6 只迁移窗口宿主，U7 迁移正式内容；两轮都复用现有读写与任务状态，不建立第二份编辑事务 |
