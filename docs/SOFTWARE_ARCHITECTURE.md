...
`src/renderer/App.tsx` 当前承担 0.7.7 的主要 UI 和交互编排：
- 搜索胶囊 UI 与输入提交流程。
...
`src/renderer/styles.css` 是 0.7.7 UI 的主要样式入口，包含搜索胶囊、统一窗口壳层、Settings、右键菜单、独立预览、关键词编辑、自绘滚动条、等待状态、交互动效、待机线和主题变量。Settings 底部版本号以文字按钮形式打开固定 GitHub Releases 页面，不承担自动版本检查或更新下载。普通界面默认禁止文本选择；`input`、`textarea` 和 `contenteditable` 保留文本选择、复制、剪切、粘贴和 Ctrl+A。
...
Cap7CE 0.7.7 的窗口状态是同一搜索胶囊系统的不同展开程度：
...
旧版通用 Modal 组件及其专用遮罩样式已经移除。Settings 内容结构已经进入 0.7.7 稳定状态，后续不要按旧设计稿重排页面。
...
Cap7CE 0.7.7 仍处于 0.7 搜索胶囊体系。当前稳定边界包括统一主窗口壳层与右侧控制栏、目录 / 识别状态 / 排序 / 文件格式筛选、可交互且自动淡出的自绘滚动条、三类视觉缓存统一管理、后台增量缩略图优化、可关闭的搜索框操作提示、随内容切换自适应且新会话回到屏幕中心的独立预览窗口、耗时操作等待反馈、输入框内快捷指令反馈、中文 / English / 跟随系统的运行时语言切换、具备实际注册检测和录入期暂停机制且临时冲突不改写总开关偏好的可配置全局快捷动作，以及不介入窗口状态机和业务链路的统一 Renderer 交互动效；这些能力沿用现有窗口状态机、搜索、索引和缓存边界。
