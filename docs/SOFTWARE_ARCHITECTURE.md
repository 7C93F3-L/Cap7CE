...
0.8.1 使用 Electron 43.2.0、Vite 7.3.6 与 electron-builder 26.15.3。生产与完整依赖审计均保持 0 条漏洞；升级时不得使用 `npm audit fix --force` 强制降级或跨越兼容边界。
...
`src/renderer/App.tsx` 当前承担 0.8.1 的主要 UI 和交互编排：
- 搜索胶囊 UI 与输入提交流程。
...
- 目录、识别状态、排序、文件格式标签及右键显隐菜单；正式搜索结果页按“目录、排序、识别状态、文件格式”排列可见标签，skim 继续按“浏览模式、排序、面包屑”排列，使排序入口在两种内容视图中均位于第二项。
...
- micro / mini / normal 搜索结果，以及未识别、解析失败、文件错误的紧凑响应式卡片视图。结果页输入框只在用户把非空关键词清空后延迟执行当前筛选范围的空查询；重新输入、显式提交、筛选变化或视图卸载会取消等待，快捷命令、skim 与 Settings 的程序化清空不进入该路径。
...
Cap7CE 0.8.1 的窗口状态是同一搜索胶囊系统的不同展开程度：
...
自动缓存优化只预生成 `capth300` 搜索缩略图，不批量生成预览图或模型输入图。主进程不在启动、目录列表加载或重新启用开关时扫描源目录，而是复用用户搜索、目录扫描和索引刷新产生的扫描结果，以源路径、文件大小、精确修改时间及内部缓存/渲染版本计算目标缓存键；缓存目录文件名清单只在首次需要筛选候选时读取并在内存中维护，缓存数量与大小也延后到用户打开 Settings 时统计。仅将缺少完整图片与元数据文件的新增、修改或版本失效项目加入队列。内部缓存或渲染版本变化后，后续用户触发的目录扫描会渐进补齐新版本缓存。任务按当前排序偏好串行执行，只有主窗口聚焦且处于 results 或 skim 内容视图时才领取新项目；进入 Settings、standby、后台或预览窗口后暂停，并允许当前单文件原子写入结束。AI 识别期间继续使用独立暂停原因；用户关闭自动优化或确认清理缓存后停止领取新任务，等待当前单文件结束再清理。
...
skim 显示范围通过现有偏好系统持久化为 `skim`、`all`、`custom` 三种模式；自定义模式保存扩展名集合及 Windows 隐藏文件开关。自定义格式配置以“圆点 + 类别文字”的完整按钮切换整类，悬停统一使用主题色；扩展名按钮选中后使用副色背景，并根据副色亮度自动选择深色或浅色文字。目录读取仍只发生在用户明确浏览时，模式切换在 Renderer 对当前一级目录结果进行过滤，不启动后台递归扫描。Windows 端使用 `fsutil` 枚举盘符、单次 `cmd vol` 读取卷标，并通过 Unicode `dir /a:h` 按当前目录批量读取隐藏属性；不在导航热路径启动 PowerShell，也不逐文件启动外部进程。
...
旧版通用 Modal 组件及其专用遮罩样式已经移除。Settings 内容结构已经进入 0.8.1 稳定状态，后续不要按旧设计稿重排页面。
...
Cap7CE 0.8.1 在既有搜索胶囊体系上继续完善 skim、多格式显示和后台低占用行为。当前稳定边界包括统一主窗口壳层与右侧控制栏、micro / mini / normal 搜索结果、独立但复用同一壳层的 skim 浏览视图、视觉与非视觉文件的确定性文件名 / 根目录 / 相对路径搜索、15 秒可取消扫描快照、原生视觉与 Windows Shell 按需生成的独立 skim 缓存、视觉 / 文件信息 / 文本 / 音频 / 视频 / PDF 共用预览窗口、Office 基础正文或临时 PDF 预览、目录拖入添加与文件/文件夹原生拖出、虚拟化 skim 网格、共享自绘滚动条、主题感知拾色器、后台增量缩略图优化、输入框内快捷指令反馈、中英文运行时语言，以及具备真实注册检测的可配置全局快捷动作。0.8.1 进一步加入用户触发的更新下载引导、可关闭系统通知、运行时与模型取消选择、skim 显示范围持久化配置、空闲资源释放和预览窗口时序保护；这些能力继续保持主窗口状态机、正式视觉索引、AI 识别、通用文件目录层和正式/skim 缓存的职责边界。
