...
- skim 作为主 Renderer 内的临时浏览视图复用现有窗口壳层，不创建新主窗口状态；从磁盘列表开始，只读取当前一级文件夹和中央 skim 白名单内的文件。通用条目携带路径、扩展名、大小、修改时间、已添加目录归属、格式能力及临时状态；混合网格通过 `skim-*` 与 `format-*` 图标命名空间显示视觉文件和项目关联非视觉文件。文件夹与文件支持单选、Ctrl/Shift 多选、活动项、共用右键菜单和多文件原生拖出；添加操作复用统一目录服务。
...
搜索交互包括：
- 空搜索显示当前范围内全部文件。
- 文件扫描完成后即可按文件名命中结果，不要求先执行 AI 识别；服务层将未索引扫描结果与 SQLite 搜索结果合并并去重。
- 中文关键词搜索。
- 与 / 或逻辑。
- 目录标签筛选。
- 识别状态筛选。
