...
- 左侧栏提供查看范围、目录、排序和 AI 增强控制；目录项在展开与收起状态下均通过 150 ms 延迟的非交互浮层显示名称、文件数量和路径，其中名称与复用顶部统计文案的文件数量位于同一行，过长路径保留首尾并从中间省略；“全部目录”没有路径，其第二行直接显示当前已加载的已添加目录数量。浮层与右键管理菜单互斥，并在移出、按下或列表滚动时立即关闭，不依赖延迟较长的系统 `title`。正式搜索入口与结果页不再提供识别状态、动态文件格式标签或显隐菜单，扩展名继续作为普通搜索词使用。skim 按“查看范围、排序、面包屑”排列，使排序入口在两种内容视图中均位于第二项。
