...
1.0.4 使用 Electron 43.2.0、Vite 7.3.6 与 electron-builder 26.15.3。生产与完整依赖审计均保持 0 条漏洞；升级时不得使用 `npm audit fix --force` 强制降级或跨越兼容边界。
...
- `edgeCollapseWindowMode.ts`、`dockedShellController.ts` / `dockedShellAutomation.ts` 管理主窗口和预览窗口自身的边缘收起：偏好关闭时恢复原生最大化与 Windows Snap；偏好开启时先退出已有最大化或 Snap 几何，再停用最大化资格并启用收起，Settings 始终保持普通窗口能力。窗口边界只有准确接触或越过非任务栏物理屏幕外沿时才建立会话，距离屏幕仍有 1 DIP 即不响应；相邻显示器覆盖的接缝不算外沿，窗口完整进入另一显示器后按新屏幕重新计算。手动拖动结束会立即确认最终边界；如果窗口已经越界，则先保持原尺寸平移回准确贴边位置并进入 programmatic move guard，随后鼠标离开才把原生 BrowserWindow 一次性移到显示器外，并让真实窗口边沿与鼠标恢复区统一保留 2 DIP；恢复时撤销临时 `floating` 层级并在不抢焦点的情况下提升至普通窗口前方。最小化只暂停轮询、释放临时层级并保留收起会话，任务栏还原后恢复完整窗口；真正隐藏才清除会话。两类窗口复用相同控制器，但固定、停靠与 collapsed 状态相互独立；固定窗口继续使用独立的持久置顶语义并暂停自身收起。窗口移动 / resize 与原生文件拖出期间统一抑制自动收展，程序位置变化进入对应 move / resize guard，独立 line 始终隐藏；显示器拔插、分辨率或任务栏工作区变化时统一取消临时层级和收起会话，并把完整窗口夹回当前可见工作区。
...
- 通过 Electron `Notification` 发送静音的 Windows Toast，使用固定 `Cap7CE` AppUserModelID、安装器创建的同身份开始菜单快捷方式和 Cap7CE 图标接入系统通知平台；安装版不回退传统托盘气泡，系统不支持或注册失败时只记录不含正文的最小诊断。主界面按钮、全局快捷动作、托盘、Toast 激活和 Preview 统一复用同一个独立 Settings 打开动作，避免多个 Renderer 同时持有设置事务。
...
| `src/renderer/settings-window/SettingsWindowApp.tsx` / `useSettingsWindowController.ts` / `useSettingsConfirmation.ts` / `SettingsWindowUpdateControl.tsx` / `SettingsConfirmationDialog.tsx` / `SettingsWindowAccessibility.css` | 独立 Settings 的八分类页面组合、本地条目筛选、正式偏好与领域任务控制器、确认事务生命周期、应用更新状态、共享确认弹窗适配，以及键盘焦点 / 长文案 / 减少动态效果；直接复用 preload 白名单，不持有第二份持久化、目录、缓存、快捷键、诊断、运行时或模型服务。窗口材质通过白名单偏好链选择 Acrylic / Mica，并同步刷新 stable 三窗口或回退安全纯色；字体大小通过正式偏好链在 12–16px 五档间选择并同步 stable 窗口；新版快捷列表只显示五项窗口动作与目录循环并读取 stable 专用快捷配置 |
| `lineWindowController.ts` | 复用同一个透明、不可聚焦的 line BrowserWindow；line 位置只根据当前显示器任务栏占用的 work area 方向推断，无法判断时回退底部，不跟随主窗口停靠记录；按动态 placement 在上下显示横线、左右显示竖线，根据真实窗口尺寸二次校正 bounds / shape，并向专用 Renderer 同步方向及实际可见状态；隐藏时暂停渐变动画，显示时以合成层位移保持原有颜色、方向与 3 秒流动节奏 |
| `edgeCollapseWindowMode.ts` / `dockedShellController.ts` / `dockedShellAutomation.ts` / `previewDockedShell.ts` / `windowLayerController.ts` | 主窗口与预览窗口共用的边缘收起窗口能力、控制器、通用生命周期装配、预览专用适配与窗口层级仲裁：按偏好互斥切换原生最大化 / Windows Snap 与边缘收起，只接受准确接触或越过非任务栏物理屏幕外沿的窗口边界，并排除多显示器接缝；管理各自 dock session、手动拖动结束确认及越界窗口原尺寸贴边归正、固定暂停、最小化暂停及任务栏还原展开、收起态展开 bounds 更新、以原生越界 bounds 保留 2 DIP 真实边沿并以同一 2 DIP 屏幕外沿作为即时恢复区，同时独立协调持久固定、收起临时浮动层级与 line 层级；还负责自适应鼠标轮询、交互抑制、阴影恢复、programmatic move / resize guard 与显示器配置变化后的安全展开夹取；不新增 Renderer IPC，额外调试快捷键仅主窗口开发版注册 |
...
| `searchScanSnapshotService.ts` | 按已添加目录维护 15 秒内存扫描快照，合并并发扫描请求，并处理显式扫描注入、目录变更失效和主窗口隐藏 / 最小化 / line 生命周期取消；主窗口仍可见但暂时失焦时保留当前扫描 |
...
| `thumbnailService.ts` | 搜索结果缩略图缓存入口；Sharp 支持的正式缩略图统一经单任务队列串行渲染，避免大量损坏或加密文件并发进入 Windows 原生解码错误路径时触发 libvips 竞态；交互式失败按源文件版本在当前进程内抑制，文件变化、目录 / 文件缓存删除或手动清理后解除，最小化及隐藏窗口只丢弃尚未领取的交互式请求，不中断已开始的单文件渲染或后台优化 |
...
- 应用：退出进程、通过固定主进程链接在系统浏览器中打开 GitHub Releases；用户点击检查更新时，由主进程查询固定 Cap7CE GitHub Releases API，只接受版本、标签、上传状态、大小与 SHA-256 摘要均匹配的 Windows x64 NSIS 安装器，并在行内提示更高版本。用户再次点击后才下载到 `%LOCALAPPDATA%\Cap7CE\updates`，支持暂停、断网及重启续传；完整校验通过后由用户确认安装，主进程成功交给系统 Shell 启动安装器，再通过统一退出流程安全结束旧进程。Renderer 不传入任意外部 URL，也不能指定下载或安装路径。
...
跨窗口共用的主题渐变变量由 `src/renderer/accentGradient.css` 持有；Preview 的偏好订阅、系统主题解析和外观变量计算由 `src/renderer/preview/usePreviewAppearance.ts` 持有，`PreviewWindowApp.tsx` 只消费其已解析的主题、材质和样式结果。

`src/renderer/App.tsx` 当前承担 1.0.4 的应用级状态和交互编排，产品主窗口的展示结构由 `StableUiRoot` 持有：
...
- 缩略图 hover / selected 黑色蒙版、单选、多选、键盘焦点；Results 专用 `useResultsSpaceHold.ts` 持有 350 ms 按住控制器、视觉状态、长按后的释放拦截及失焦 / 卸载清理，`ResultsView` 只决定开始时的选择快照，以及选择、视图或浮层变化时采用完整取消还是仅取消待定动作。计时必须跨结果数据及回调刷新保持，只有真实失焦、选择变化、视图切换或其他浮层出现才取消，避免视觉进度完成但动作丢失；Preview 保留自身不同的松键执行语义，不与 Results 强制合并。搜索结果与 skim 的非盘符选择支持 `Ctrl+C` 写入 Windows 文件剪贴板，单选、多选及文件夹共用同一主进程校验入口，输入框、文本域、下拉框和可编辑内容不拦截原生文字复制。
- 顶部结果状态继续由 `App.tsx` 根据搜索状态、当前目录统计与查询条件组合为纯展示节点，并通过稳定宿主的 `overlayContent` 插槽传入；背景交互锁同样由顶层对话框状态派生后传给 `StableUiRoot`。两者属于应用级状态组合与顶层交互编排，不应为了缩短入口而让纯展示组件反向读取搜索 / 目录状态，或新增只转发属性的中间模块。
...
U4 由 `stableUiRendererTypes.ts` 和 `stableSidebarTypes.ts` 定义显式展示适配契约，`App.tsx` 继续持有搜索、AI、目录状态和正式事务，只向 `StableShellSidebar.tsx` 传递受控值与动作。侧栏的排序、查看范围、目录筛选、系统选择添加、拖入确认、冲突替换、打开目录、行内重命名和删除均复用既有搜索、文件与目录链；新版模块不直接读取 preload，也不修改目录服务、SQLite 或 AI 任务逻辑。AI 增强入口由线框 / 实心图标和副文案表达状态，不保留重复滑动开关。`StableSidebar.css` 独立持有控制行、目录滚动、选中 / 悬停胶囊、40px 紧凑栏和底部动作布局，目录列表复用共享自动隐藏自绘滚动条；宽度仍是 `StableMainShell.tsx` 的会话状态。`StableUiIcon.tsx` 只负责从独立 SVG 资产中选择新版线框 / 实心、目录开合和旧版升降序形态并通过共用 `SvgIcon` 内嵌，不读取业务状态。Skim 底部动作在 U5 前只切换占位插槽，Settings 动作在 U6 独立宿主完成前暂时进入现有设置页。该轮抽取共用目录确认层后把 `App.tsx` 自动上限从 3334 行降到 3330 行，并为四个新增侧栏职责模块建立独立体量守门。
...
此前完成的分阶段架构整理继续以体量与依赖守门约束运行时所有权。`scripts/architecture-boundaries-check.cjs` 将体量边界分为两类：`strictMaxLines` 对守门脚本自身、`App.tsx`、`styles.css`、`electron/main.ts`、Renderer 入口、窗口控制器及较大的领域协调器使用逐文件硬上限；`maintenanceMaxLines` 对职责已经明确的小型领域组件、Hook、契约和局部样式使用静态维护阈值，50 行以内只接受 10 行档，50 至 200 行只接受 25 行档。阈值均明确写入基线，运行时不会按实际行数自动放宽；跨过阈值时仍须先审计职责，不能通过压缩排版或机械拆分文件绕过。守门同时禁止 Renderer 引入 Electron / Node、领域模块反向依赖顶层装配文件，以及在 `main.ts` 继续新增非窗口生命周期 IPC，并为搜索、目录、Skim、Settings、Preview、文件菜单、拖放、快捷键、窗口固定与隐藏恢复建立正式源码锚点；所有权移动时必须同步迁移映射，不能让旧链悄然消失后在新版组件中复制。检查通过 `test:architecture-boundaries` 接入完整测试；后续每完成一个真实职责拆分，应同步降低对应硬上限或维护阈值并收缩 legacy main IPC 白名单，守门也会拒绝已经不再对应真实直连 channel 的过期豁免。该机制用于阻止复杂度重新堆回单体入口，不代替构建、集成测试和窗口人工回归。

A5 已把运行时与模型、临时反馈、视口只读指标、系统主题、窗口置顶、skim 目录读取与 stable Skim 浏览历史等具有单一所有权的状态簇迁入 `src/renderer/controllers/`。`useSkimNavigationHistory.ts` 仅在目录读取成功后提交当前位置，按实际访问顺序提供后退与前进，并以请求代次拒绝双击或快速切换产生的迟到历史；面包屑和目录入口仍只负责发起现有读取。stable 壳层的 Skim 快捷指令请求由 `useStableSkimCommands.ts` 产生，显示状态与请求消费由 `useStableSkimVisibility.ts` 持有；目录右键菜单由 `StableDirectoryFlyout.tsx` 独立管理排序、打开目录、编辑与删除动作，`directoryListActions.ts` 负责受限的目录移动 IPC 与刷新桥接。搜索、应用级页面导航、关键词编辑、目录扫描、缓存确认及窗口恢复仍由 App 顶层编排；其中同时跨越多个 Effect、确认或取消语义的状态簇依据停止条件保留原位，不为减少行数强拆。
...
关键词编辑卡片的主题纯色蒙版由 `src/renderer/dialogs/KeywordEditorBackdrop.tsx` 与 `KeywordEditorBackdrop.css` 独立持有；主窗口只显示浮层卡片，不再叠加白色或黑色蒙版。主窗口的关键词编辑与确认面板由 `StableUiRoot` 统一下发背景交互锁，独立 Settings 的确认面板在自身状态边界只锁定设置内容并保留顶部纯拖动区；`useInertElement.ts` 只负责把这些状态映射到对应元素的浏览器原生 `inert` 属性。Settings 专用 `useSettingsConfirmation.ts` 统一持有确认对象、忙碌状态、打开 / 取消 / 确认事务、页面隐藏清理和异步身份保护；Settings 隐藏或最小化时清除未提交确认，进行中的旧动作不会关闭随后打开的新确认。锁定期间主窗口结果与 Skim 同步退出窗口级键盘活动，所有 `DialogShell` 定位容器只从 40 DIP 原生标题栏下方覆盖内容区并承担居中布局，浮窗表面自身维持 `no-drag`。共用卡片在 `KeywordEditorCard.css` 内采用 80% 主题表面、12px 圆角、背景模糊和文件摘要层级，输入与保存语义保持不变，不把这些样式重新计入全局入口。
...
搜索结果统一由 `src/renderer/results/VirtualResultGrids.tsx` 承担普通虚拟网格；ResizeObserver、requestAnimationFrame 节流、横纵滚动映射、首屏位置恢复、选中项显露和可见项 overscan 保持共用。选择集合、键盘导航、右键菜单和预览状态仍由 ResultsView 持有并通过 Props 回调传入，空格短按 / 长按及释放保护的局部生命周期由同领域 `useResultsSpaceHold.ts` 持有，共用网格尺寸与显露计算由 `virtualGridLayout.ts` 提供确定性测试。
...
Cap7CE 1.0.4 的正式窗口状态已收口为主窗口显示与隐藏生命周期：
...
系统通知采用明确的任务结果策略：首次后台运行提示按用户数据只请求一次，并使用当前实际可用的搜索快捷键；自动缓存优化按实际工作时长判断，对处理过内容且超过 1 分钟的每个完整批次提示，不受主窗口、Settings 或 Preview 的前台状态影响，也不跨批次设置冷却。按需 AI 搜索以搜索胶囊状态和临时提示反馈，不发送旧全目录任务完成通知。系统通知关闭、短任务和空任务仍不提示。Windows 安装版仅使用现代 Toast，并以 `Notification.isSupported()` 预检；`show` / `failed` 与激活注册异常只写入事件类型、打包状态和错误，不记录通知标题或正文。开发态没有正式开始菜单身份时明确跳过，不用 tray balloon 掩盖身份问题。
...
- 搜索结果与 Skim 的右键菜单和 `Ctrl+Shift+C` 共用复制路径及成功反馈动作，不分别维护提示逻辑。
- 拖拽到资源管理器、桌面或外部应用。
...
- 快捷动作：显示并聚焦主窗口、隐藏到 line、复位主窗口、切换标准窗口 / 边缘收起模式、展开或收起 Skim、打开 Settings 与窗口内目录切换。
- 全局快捷键开关。
- 快捷指令查看列表和解析开关。
- llama.cpp 版本配置。
- 视觉模型配置。
...
默认全局窗口快捷动作依次为 Alt + 反引号、`Alt+1`、`Alt+2`、`Alt+3`、`Alt+4`、`Alt+5`，分别显示并聚焦主窗口、隐藏主窗口并显示 line、以默认大小和位置复位主窗口、切换标准窗口 / 边缘收起模式、展开或收起 Skim、打开 Settings。Settings 按相同顺序先排列四项主窗口控制，再排列 Skim 与 Settings；每项允许清除为明确的“未设置”状态，空绑定不参与注册、占用探测或冲突提示，重新录入和“恢复默认”仍沿用同一偏好链路。缺少“切换窗口模式”的旧快捷配置整组回落到当前默认值，不保留旧映射。另有默认 `Alt+Q` 的“目录切换”只在主窗口搜索结果界面生效，不注册为系统级全局快捷键；它按“所有已添加目录 → 各已添加目录 → 所有已添加目录”稳定循环，保留其他搜索条件并复用手动目录标签的刷新与输入框临时反馈；所有目录选择入口把新目录作为独立浏览上下文并从结果顶部开始，单纯排序继续保留当前位置，按键自动重复不触发连续切换。快捷指令继续独立生效，不与快捷动作互相替代。
...
旧版通用 Modal 组件及其专用遮罩样式已经移除。Settings 内容结构已经进入 1.0.2 稳定状态，后续不要按旧设计稿重排页面。
