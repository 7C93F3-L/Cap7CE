...
A6 首先由 `test:ipc-registration` 固定领域注册器的 channel、参数转发和 sender 授权语义；在任何既有外围 IPC 搬迁前先建立可重复验证的注册边界。文件打开、路径定位、文字/系统文件剪贴板、回收站删除和原生拖拽六个 channel 已迁入 `electron/fileIpc.ts`，其服务、平台状态、文案和允许访问剪贴板的主/预览 sender 均由 `main.ts` 显式注入；对应 `test:file-ipc` 固定注册清单、参数过滤、失败结果和 sender 行为。llama.cpp 运行时与 GGUF 模型的读取、选择、启动和停止七个 channel 迁入 `electron/runtimeModelIpc.ts`，继续复用既有 store/manager，并保持运行时处于 starting/running 时禁止切换版本或模型的规则；`test:runtime-model-ipc` 固定注册清单、调用顺序和禁止切换边界。只读写偏好存储且不触发窗口或后台服务副作用的读取、skim 排序、操作提示、快捷指令、标签显隐、自定义查看和边栏状态八个 channel 迁入 `electron/preferenceIpc.ts`；主题、line、边缘吸附、登录启动、系统通知、自动缓存与全局快捷键等带运行期副作用的偏好仍由 `main.ts` 编排。缓存统计、优化状态和主 Renderer 内容/网格活动信号五个 channel 迁入 `electron/cacheActivityIpc.ts`；主窗口 sender 边界、实际活动状态及缩略图优化 pause/resume 动作由 `main.ts` 注入，清理授权和执行仍留在原装配层。领域模块不得反向导入 `main.ts`，主进程继续负责依赖注入和生命周期装配。
