...
- 搜索结果、skim 与独立预览窗口共用“查看 / 操作”两级右键菜单；一级菜单顶部是无按钮样式的紧凑文件摘要并保持两个动作入口，二级菜单根据可用空间向左或向右展开并限制在当前窗口内。复制路径通过受限主进程剪贴板入口写入绝对路径，多选时使用 CRLF 逐行写入；非视觉文件不因此获得关键词编辑或删除资格，skim 也不扩大既有删除权限。
