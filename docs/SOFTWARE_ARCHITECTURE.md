...
- 旧 Settings 页面保留当前内容结构，使用新版胶囊视觉和行内任务状态；稳定 UI 独立 Settings 在 U7 按八个新版分类组合现有偏好、目录和任务能力，两种入口由偏好 / 目录变更广播保持正式状态同步。新版较详细文案优先，尚未一一对应的增删改项允许后续补齐；窗口材质由 stable Settings 在 Acrylic / Mica 间选择并实时同步三窗口，纯色只作为失败回退。
