...
0.9.4 使用 Electron 43.2.0、Vite 7.3.6 与 electron-builder 26.15.3。生产与完整依赖审计均保持 0 条漏洞；升级时不得使用 `npm audit fix --force` 强制降级或跨越兼容边界。
...
`src/renderer/App.tsx` 当前承担 0.9.4 的主要 UI 和交互编排：
...
Cap7CE 0.9.4 的窗口状态是同一搜索胶囊系统的不同展开程度：
...
旧版通用 Modal 组件及其专用遮罩样式已经移除。Settings 内容结构已经进入 0.9.4 稳定状态，后续不要按旧设计稿重排页面。
