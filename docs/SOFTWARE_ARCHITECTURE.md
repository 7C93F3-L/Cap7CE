...
- 搜索结果右键菜单。
- 独立预览窗口的打开入口和预览数据协调；`PreviewWindowApp.tsx` 通过 provider 字段渲染视觉内容、文件/文件夹信息、纯文本、音频或视频并复用 `WindowControlRail`。文件夹统计只在显式预览后启动，切换项目、关闭预览或退出 skim 时取消；已提交的单次文件系统调用允许返回，但任务停止领取新目录并忽略迟到更新。音视频切换、窗口隐藏或关闭时显式暂停并释放当前媒体源。
- 单选 / 多选关键词编辑面板与批量结果反馈。
- Settings 页面与配置项展开。
