...
目录列表、目录名称修改、选择添加和外部候选添加四个入口迁入 `electron/directoryManagementIpc.ts`；模块固定请求规范化、取消结果，以及操作完成后统一补齐 SQLite 索引计数。目录选择窗口、目录存储、索引迁移队列与失败回滚仍由 `main.ts` 注入。刷新计数与删除目录继续因涉及扫描和缓存队列而留在主装配层。
