...
- 轻阅：枚举本机文件系统盘符、读取当前位置的直接子项，以及取消尚未完成的读取任务。
...
- skim 作为主 Renderer 内的临时浏览视图复用现有窗口壳层，不创建新 `BrowserWindow` 或窗口状态；首期从磁盘列表开始，只显示当前一级文件夹和已支持的视觉文件。
- 缩略图 hover / selected 黑色蒙版、单选、多选、键盘焦点。
- 搜索结果右键菜单。
- 独立预览窗口的打开入口和预览数据协调；`PreviewWindowApp.tsx` 渲染预览内容并复用 `WindowControlRail`。
- 单选 / 多选关键词编辑面板与批量结果反馈。
- Settings 页面与配置项展开。
