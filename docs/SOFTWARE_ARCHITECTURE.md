...
关键词编辑卡片的主题纯色蒙版由 `src/renderer/dialogs/KeywordEditorBackdrop.tsx` 与 `KeywordEditorBackdrop.css` 独立持有；主窗口只显示浮层卡片，不再叠加白色或黑色蒙版。主窗口的关键词编辑与确认面板由 `StableUiRoot` 统一下发背景交互锁，`useInertElement.ts` 只负责把该状态映射到 Portal 标题栏和主内容的浏览器原生 `inert` 属性；锁定期间结果与 Skim 同步退出窗口级键盘活动，确认层的全屏容器只承担居中布局。共用卡片在 `KeywordEditorCard.css` 内采用 80% 主题表面、12px 圆角、背景模糊和文件摘要层级，输入与保存语义保持不变，不把这些样式重新计入全局入口。
