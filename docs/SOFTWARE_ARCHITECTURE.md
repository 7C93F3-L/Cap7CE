...
目录列表、目录名称修改、选择添加、外部候选添加和刷新文件计数五个入口迁入 `electron/directoryManagementIpc.ts`；模块固定请求规范化、取消结果、扫描快照与索引写入顺序，以及操作完成后统一补齐 SQLite 索引计数。目录选择窗口、目录存储、扫描器、索引迁移队列与失败回滚仍由 `main.ts` 注入。删除目录继续因涉及缓存暂停、队列丢弃与文件清理而留在主装配层。
