...
0.9.0 使用 Electron 43.2.0、Vite 7.3.6 与 electron-builder 26.15.3。生产与完整依赖审计均保持 0 条漏洞；升级时不得使用 `npm audit fix --force` 强制降级或跨越兼容边界。
...
`src/renderer/App.tsx` 当前承担 0.9.0 的主要 UI 和交互编排：
- 搜索胶囊 UI 与输入提交流程。
...
`src/renderer/styles.css` 是 0.9.0 UI 的主要样式入口，包含搜索胶囊、统一窗口壳层、Settings、右键菜单、自绘拾色器、独立预览、关键词编辑、自绘滚动条、等待状态、交互动效、待机线和主题变量。Settings 可操作按钮统一提供经过审校的本地化 `title` 悬停说明；状态开关根据当前状态描述下一次点击结果。Settings 底部版本号仍以文字按钮形式打开固定 GitHub Releases 页面；视觉模型下方的“版本更新”行仅在用户点击时检查更新，发现新版后先显示版本号并将按钮切换为“立即下载”，再次点击才在行内显示下载进度。自动替换仅在打包版启用：更新助手先解压并校验 ZIP 内的 `Cap7CE.exe` 与 `resources/app.asar`，写出就绪信号后主进程才退出；助手启动失败、提前退出或未在时限内就绪时，主进程保持运行并删除本轮临时文件。助手接管后等待旧进程退出，再备份当前程序目录、复制新版并保留用户自行放置的 `models` 与 `llama.cpp`；新版无法稳定启动时恢复备份并重启旧版，失败时保留更新日志，成功后删除备份及临时下载。该流程不后台检查或下载，也不修改 `%APPDATA%\Cap7CE` 用户数据；未来安装包更新策略应继续复用相同的检查、确认与进度状态，只替换主进程执行器。普通界面默认禁止文本选择；`input`、`textarea` 和 `contenteditable` 保留文本选择、复制、剪切、粘贴和 Ctrl+A。
...
Cap7CE 0.9.0 的窗口状态是同一搜索胶囊系统的不同展开程度：
...
旧版通用 Modal 组件及其专用遮罩样式已经移除。Settings 内容结构已经进入 0.9.0 稳定状态，后续不要按旧设计稿重排页面。
