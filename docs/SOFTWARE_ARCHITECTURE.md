...
| `skimBrowseService.ts` | skim 的磁盘枚举、当前一级目录读取、视觉格式过滤、面包屑构建与协作式取消 |
...
- skim 作为主 Renderer 内的临时浏览视图复用现有窗口壳层，不创建新主窗口状态；首期从磁盘列表开始，只显示当前一级文件夹和已支持的视觉文件。文件夹与文件支持单选、Ctrl/Shift 多选、活动项、共用右键菜单和多文件原生拖出；添加操作复用统一目录服务。
