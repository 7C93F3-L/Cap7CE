...
Cap7CE 0.8.1 在既有搜索胶囊体系上继续完善 skim、多格式显示和后台低占用行为。当前稳定边界包括统一主窗口壳层与右侧控制栏、micro / mini / normal 搜索结果、独立但复用同一壳层的 skim 浏览视图、视觉与非视觉文件名搜索、按需生成的独立 skim 缓存、视觉 / 文件信息 / 文本 / 音频 / 视频共用预览窗口、目录拖入添加与文件/文件夹原生拖出、虚拟化 skim 网格、共享自绘滚动条、主题感知拾色器、后台增量缩略图优化、输入框内快捷指令反馈、中英文运行时语言，以及具备真实注册检测的可配置全局快捷动作。0.8.1 进一步加入用户触发的更新下载引导、可关闭系统通知、运行时与模型取消选择、skim 显示范围持久化配置、空闲资源释放和预览窗口时序保护；这些能力继续保持主窗口状态机、正式视觉索引、AI 识别、通用文件目录层和两套缓存的职责边界。
