...
0.9.1 使用 Electron 43.2.0、Vite 7.3.6 与 electron-builder 26.15.3。生产与完整依赖审计均保持 0 条漏洞；升级时不得使用 `npm audit fix --force` 强制降级或跨越兼容边界。
...
`src/renderer/App.tsx` 当前承担 0.9.1 的主要 UI 和交互编排：
- 搜索胶囊 UI 与输入提交流程。
...
添加目录后用于刷新目录文件数量的递归扫描同时把全部已登记格式写入 `files` 通用目录层，并把正式视觉文件写入 `images` 层，但不会自动启动 AI。Settings 顶部“识别状态”仅统计正式视觉文件：关键词字段非空即为“已识别”，无论关键词来自 AI 还是用户手工录入；关键词为空即为“未识别”，因此“全部文件”始终等于“已识别 + 未识别”。目录行的文件数量仍统计全部已登记格式，混合目录中可能大于顶部视觉文件总数。
...
Cap7CE 0.9.1 的窗口状态是同一搜索胶囊系统的不同展开程度：
...
旧版通用 Modal 组件及其专用遮罩样式已经移除。Settings 内容结构已经进入 0.9.1 稳定状态，后续不要按旧设计稿重排页面。
...
Cap7CE 0.9.1 在既有搜索胶囊体系上完成多格式浏览、搜索与内容预览能力扩展。当前稳定边界包括统一主窗口壳层与右侧控制栏、micro / mini / normal 搜索结果、独立但复用同一壳层的 skim 浏览视图、123 种已登记格式的确定性文件名 / 根目录 / 相对路径 / 手工关键词搜索、15 秒可取消扫描快照、原生视觉与 Windows Shell 按需生成的独立 skim 缓存、视觉 / 文件信息 / 文本 / Markdown / 字体 / 归档 / EPUB / MOBI / 音频 / 视频 / PDF / Office 共用预览窗口、目录拖入添加与文件/文件夹原生拖出及系统剪贴板复制、虚拟化 skim 网格、共享查看范围、自绘滚动条、主题感知图标与拾色器、持续推进的后台增量缩略图优化、输入框内快捷指令和标签切换反馈、窗口内目录循环切换、中英文运行时语言，以及具备真实注册检测的可配置全局快捷动作。只有 15 种正式视觉格式进入缩略图与 AI 识别，其他格式保持通用搜索与预览边界；正文全文、OCR、AI 深度索引和相关性排序仍未接入。
