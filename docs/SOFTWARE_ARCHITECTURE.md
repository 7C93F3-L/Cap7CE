...
- 搜索胶囊 UI 与统一输入提交流程；快捷指令优先，其次识别盘符或 UNC 开头的完整 Windows 目录路径并在主进程确认真实可读后进入 skim，其余内容执行正式搜索。目录校验失败时保留原输入并显示临时提示。
