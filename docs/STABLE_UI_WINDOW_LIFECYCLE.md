...
通用文件与文件夹在 stable Preview 内容区保留文件名、类型、路径、修改时间、大小和索引范围等基础摘要，确保信息边栏收起时仍可独立阅读；内容区使用既有格式图标、新版半透明卡片与中性字段层级。普通文件、扩展失败信息和文件夹分别按固定摘要结构请求对应内容尺寸，使窗口一次成形且不持续干预自由缩放；长路径或详情超出可用空间时只滚动当前面板，不触发多文件预览导航。旧宿主继续使用原有信息面板和尺寸。
