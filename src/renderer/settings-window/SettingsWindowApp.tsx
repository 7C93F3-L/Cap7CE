...
import { useInertElement } from "../stable-ui/useInertElement";
import { getTextColorForBackground } from "../appearance";
import { getFileContextMenuStyle } from "../fileContextMenuShared";
import { formatCacheSize, formatThumbnailOptimizationStatus } from "../formatting";
...
const SettingsWindowApp = () => {
  ...
  const { dialog, dialogBusy, openConfirmation, cancelDialog, confirmDialog } = useSettingsConfirmation();
  const [systemDark, setSystemDark] = useState(() => window.matchMedia("(prefers-color-scheme: dark)").matches);
  const scrollRef = useRef<HTMLDivElement | null>(null);
  const settingsShellRef = useInertElement<HTMLDivElement>(dialog !== null);
  ...
  const renderCategory = (category: CategoryId) => {
    ...
    return <><SettingsSection title="stableSettings.section.application"><SettingCard title="settings.versionUpdate" description="stableSettings.desc.update" query={normalizedQuery} className="cap-stable-settings-card-expanded"><SettingsWindowUpdateControl requestConfirmation={openConfirmation} /></SettingCard><SettingCard title="stableSettings.license" description="stableSettings.desc.license" query={normalizedQuery} /></SettingsSection><SettingsFooter /></>;
  };
  ...
  return <div className={`cap-settings-window-foundation theme-${effectiveTheme}`} data-window-material={preferences.windowMaterial} data-language={getActiveLanguage()} style={menuStyle}>
    ...
    <div ref={settingsShellRef} className="cap-stable-settings-shell">
    ...
    {dialog && <SettingsConfirmationDialog message={dialog.message} busy={dialogBusy} confirmLabel={dialog.confirmLabel} onCancel={cancelDialog} onConfirm={() => void confirmDialog()} />}
  </div>;
};
