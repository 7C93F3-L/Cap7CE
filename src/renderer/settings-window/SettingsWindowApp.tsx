...
const categoryDefinitions: Array<{ id: CategoryId; label: TranslationKey; short: string }> = [
  ...
  { id: "browse", label: "stableSettings.category.browse", short: "▣" },
...
const categorySearchKeys: Record<CategoryId, TranslationKey[]> = {
  ...
  browse: ["stableSettings.category.browse", "stableSettings.section.folders", "settings.skimDisplay", "settings.standbyLine", "stableSettings.desc.folders", "stableSettings.desc.skimDisplay", "stableSettings.desc.line"],
...
const SettingsWindowApp = () => {
  ...
  const directorySummary = controller.totalFileCount === null
  ...
  const menuStyle = { "--context-menu-theme-color": preferences.appearanceColors.themeColor, "--context-menu-accent-color": preferences.appearanceColors.accentColor } as CSSProperties;
  ...
  const beginAddDirectory = async () => {
    const result = await controller.addDirectory();
    if (!result || result.cancelled || result.conflicts.length === 0) return;
    openConfirmation(t("stableSettings.directoryConflict"), () => controller.replaceDirectoryConflicts(result));
  };
  ...
  const renderCategory = (category: CategoryId) => {
    ...
    if (category === "appearance") return <>
      ...
      <SettingsSection title="stableSettings.section.labels"><SettingCard title="stableSettings.hiddenLabels" description="stableSettings.desc.hiddenLabels" query={normalizedQuery}><button type="button" className="cap-stable-settings-button" disabled={allLabelsVisible} onClick={() => void controller.updateSearchLabelVisibility({ directory: true, sort: true, format: true, skimDisplay: true, ai: true })}>{t("stableSettings.restoreLabels")}</button></SettingCard></SettingsSection>
    </>;
    if (category === "browse") return <>
      <SettingsSection title="stableSettings.section.folders"><SettingCard title="stableSettings.section.folders" description="stableSettings.desc.folders" query={normalizedQuery} className="cap-stable-settings-card-expanded">
        <div className="cap-stable-settings-directory-summary"><span>{directorySummary}</span><button type="button" className="cap-stable-settings-button" disabled={controller.isAddingDirectory} onClick={() => void beginAddDirectory()}>{t("common.add")}</button></div>
        <div className="cap-stable-settings-directories">{controller.directories.length === 0 && <p>{t("stableSettings.directoryEmpty")}</p>}{controller.directories.map((directory) => <div className="cap-stable-settings-directory" key={directory.id}>{editingDirectoryId === directory.id ? <input autoFocus value={editingDirectoryName} onChange={(event) => setEditingDirectoryName(event.target.value)} onKeyDown={(event) => { if (event.key === "Escape") setEditingDirectoryId(null); if (event.key === "Enter" && editingDirectoryName.trim()) void controller.renameDirectory(directory.id, editingDirectoryName.trim()).then(() => setEditingDirectoryId(null)); }} /> : <span><strong>{directory.name}</strong><small>{directory.path}</small></span>}<em>{directory.fileCount ?? "…"}</em><button type="button" onClick={() => { setEditingDirectoryId(directory.id); setEditingDirectoryName(directory.name); }}>{t("stableSettings.rename")}</button><button type="button" onClick={() => openConfirmation(t("stableSettings.confirmDeleteDirectory", { name: directory.name }), () => controller.deleteDirectory(directory.id))}>{t("common.delete")}</button></div>)}</div>
      </SettingCard></SettingsSection>
      <SettingsSection title="stableSettings.section.browsing"><SettingCard title="settings.skimDisplay" description="stableSettings.desc.skimDisplay" query={normalizedQuery} className="cap-stable-settings-card-expanded"><SkimDisplaySettingsRows skimDisplay={preferences.skimDisplay} onSkimDisplayChange={(next) => void controller.updateSkimDisplay(next)} /></SettingCard><SettingCard title="settings.standbyLine" description="stableSettings.desc.line" query={normalizedQuery}><SettingsToggle enabled={preferences.standbyLineVisible} onChange={(enabled) => toggle("standbyLineVisible", enabled)} /></SettingCard></SettingsSection>
    </>;
  ...
  return <div className={`cap-settings-window-foundation theme-${effectiveTheme}`} data-language={getActiveLanguage()} style={menuStyle}>
    ...
    {dialog && <div className="cap-stable-settings-dialog-backdrop" role="presentation"><section className="cap-stable-settings-dialog" role="dialog" aria-modal="true"><p>{dialog.message}</p><div><button type="button" disabled={dialogBusy} onClick={() => setDialog(null)}>{t("common.cancel")}</button><button type="button" disabled={dialogBusy} onClick={() => void confirmDialog()}>{dialogBusy ? t("common.loading") : dialog.confirmLabel ?? t("common.confirm")}</button></div></section></div>}
  </div>;
};
