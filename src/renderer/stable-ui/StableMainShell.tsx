...
const StableMainShell = ({ resultContent, sidebar, skim }: { resultContent: (active: boolean) => ReactNode; sidebar: StableSidebarProps; skim: StableSkimProps }) => {
  ...
  return (
    <section className={`cap-stable-main-shell${skimOpen ? " is-skim-open" : ""}`} style={shellStyle}>
      ...
      <section className="cap-stable-results-slot" aria-label={t("stableUi.resultsRegion")}>{resultContent(!skimOpen)}</section>
