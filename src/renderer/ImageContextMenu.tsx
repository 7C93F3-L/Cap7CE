...
const ImageContextMenu = ({
  ...
  const measurementKey = `${x}:${y}:${compact}:${header.format}:${header.fileName}:${header.primaryDetail ?? ""}:${header.details.join(",")}:${groups.map((group) => `${group.id}:${group.label}:${group.actions.map((action) => `${action.id}:${action.label}:${action.shortcut ?? ""}`).join(",")}`).join("|")}`;
  ...
  const headerTooltip = useMemo(() => [
    t("context.tooltipFile", { name: header.sourceFileName ?? header.fileName }),
    t("context.tooltipDirectory", { path: getContextMenuParentPath(header.filePath) })
  ].join("\n"), [header.fileName, header.filePath, header.sourceFileName]);
          ...
          {activeGroup && (
            ...
            >
              {activeGroup.actions.map((action) => (
                ...
                >
                  <span className="context-menu-action-label">{action.label}</span>
                  {action.shortcut && <span className="context-menu-action-shortcut">{action.shortcut}</span>}
                </button>
              ))}
            </div>
          )}
