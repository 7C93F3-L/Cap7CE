...
export type FilterChipGroup = "skimDisplay" | "directory" | "sort" | "ai";
export const standardSearchLabelGroups: FilterChipGroup[] = ["skimDisplay", "sort", "ai", "directory"];
...
export const Cap7CESearchCapsule = ({ search, directoryName, directories = [], labelVisibility, status, inputFeedback = "", inputFeedbackIsGuide = false, autoSearchOnQueryClear = false, unified = false, leadingContent, directoryGroup, skimDisplayMode = "skim", enabledLabelGroups, labelMenuEnabled = true, aiSearchEnabled = false, aiSearchBusy = false, aiSearchProgress, imageContextMenuOpen = false, inputRef, onSearchChange, onLabelVisibilityChange, onSearchOptionsChange, onSkimDisplayModeChange, onSearch, onAiSearchToggle, onImageContextMenuClose }: Cap7CESearchCapsuleProps) => {
  ...
  }, [directoryChipsOpen, labelMenuPointer, skimDisplayChipsOpen, sortChipsOpen]);
  const measuredLabelMenuPosition = useMeasuredViewportMenuPosition(labelMenuPointer, labelMenuRef, labelMenuMeasurementKey);
  const enabledGroups = new Set<FilterChipGroup>(enabledLabelGroups ?? ["directory", "sort"]);
  ...
  const hideAiSearchLabel = (event: React.MouseEvent) => {
    event.preventDefault();
    event.stopPropagation();
    updateLabelVisibility({ ...labelVisibility, ai: false });
  };
  ...
  >
    ...
    {onAiSearchToggle && enabledGroups.has("ai") && labelVisibility.ai && (
      <button
        className="cap7ce-pill cap7ce-pill-wide cap7ce-ai-search-tag"
        type="button"
