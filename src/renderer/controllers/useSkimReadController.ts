...
export const useSkimReadController = ({
  ...
  const cancel = useCallback(() => {
    ...
    if (activeVisualSessionId) {
      void window.imageEverything?.skim.cancelVisualSession(activeVisualSessionId);
    }
    setIsLoading(false);
  }, []);
  ...
  useEffect(() => () => {
    if (taskIdRef.current) {
      void window.imageEverything?.skim.cancel(taskIdRef.current);
      taskIdRef.current = null;
    }
    if (visualSessionIdRef.current) {
      void window.imageEverything?.skim.cancelVisualSession(visualSessionIdRef.current);
      visualSessionIdRef.current = null;
    }
  }, []);
